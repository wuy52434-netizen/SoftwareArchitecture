...
5. **构建前端项目**（Docker Compose 需要挂载前端构建产物）
...
| 设置项                     | 默认值 | 说明        |
| ----------------------- | --- | --------- |
| default\_borrow\_days   | 30  | 默认借阅天数    |
| max\_borrow\_count      | 5   | 最大借阅数量    |
| overdue\_fine\_per\_day | 0.5 | 逾期罚款（元/天） |
| max\_fine\_amount       | 50  | 最高罚款金额    |
...
3. **常见错误及修复**
   ...
   | 错误信息 | 原因 | 解决方案 |
   |---------|------|---------|
   | `may not have port 8080 in upstream` | `upstream` 块放在 `server` 块后面 | 确保 `upstream` 块在 `server` 块之前 |
   | `host not found in upstream "api-gateway:8080"` | api-gateway 服务未启动，Nginx 启动时尝试解析 upstream 主机名失败 | 方案一：先启动后端微服务再启动 Nginx<br>方案二：修改 nginx.conf 移除 upstream 块，直接使用 `proxy_pass http://host.docker.internal:8080/` |
   | 404 Not Found | 前端 dist 目录为空或挂载路径错误 | 确认 `frontend/dist/index.html` 存在，检查 docker-compose 挂载路径 |

4. **推荐的 nginx.conf 配置（无需依赖 upstream）**
   ```nginx
   events {
   ...
   http {
       ...
       server {
           ...
           location /api/ {
               # 使用 host.docker.internal 访问宿主机（Docker Desktop）
               # 或使用具体的服务名（需要在同一 Docker 网络）
...
### Q7: 后端微服务启动失败（no main manifest attribute）

**原因**：`spring-boot-maven-plugin` 未执行，jar 包没有被重新打包成可执行的 fat jar

**解决**：

1. **检查子模块 pom.xml 是否包含插件**
   ```xml
   <build>
       <plugins>
           <plugin>
...
4. **在 IDEA 中操作**
   - 右键父工程 `pom.xml` → Maven → Reload Project
   - 执行 Maven → Lifecycle → clean → package
